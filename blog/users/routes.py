...
@users.route("/reset_password", methods=['GET', 'POST'])  #request reset password
def reset_request():
    if current_user.is_authenticated:
        return redirect(url_for('main.home'))
...
@users.route("/reset_password/<token>", methods=['GET', 'POST'])  #reset password
def reset_token(token):
    if current_user.is_authenticated:
        return redirect(url_for('main.home'))
    user = User.verify_reset_token(token)
    if user is None:
        flash('That is an unvalid or expired token', 'warning')
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(f'Your password has been updated. You can log in', 'success')
        return redirect(url_for('users.login')) 
